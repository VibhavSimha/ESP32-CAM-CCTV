...
static void _boreProxyConn(WiFiClient &remote, WiFiClient &local) {
  ...
  while (remote.connected() && local.connected() && millis() - idle < 30000) {
    ...
    // CRITICAL FIX: Drain control connection while proxying long-lived streams (like MJPEG).
    // bore.pub sends heartbeats every 10s over _boreCtrl. If _boreProxyConn blocks _boreServe()
    // from reading _boreCtrl, the control TCP buffer overflows and bore.pub drops the tunnel after 10s!
